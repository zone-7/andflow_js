var andflow_designer = {
  ...
  showFlow: function (option) {
    ...
    var root = this;
    for(var i in opt.flowModel.actions){
      opt.flowModel.actions[i].content = null;
    }
  ...
  showDialog: function (opt) {
    ...
    $('.designer-dialog-mask').remove();
    $('.designer-dialog').remove();
    $('.designer-box').append(html);

    //dialog
    $('.designer-dialog').draggable({ handle: '.designer-dialog-header' });
    $('.designer-dialog #designer_dialog_close_btn').bind('click', function () {
      root.closeDialog();
    });
    $('.designer-dialog #designer_dialog_max_btn').bind('click', function () {
      var max = $('.designer-dialog').attr('max');
      if (max == 'true') {
        var w = $('.designer-dialog').attr('default_width') || '50%';
        var h = $('.designer-dialog').attr('default_height') || '50%';
        ...
        $('.designer-dialog #designer_dialog_max_btn').html('+');
      } else {
    ...
    if (opt.full == true) {
      ...
      $('.designer-dialog').attr('max', 'true');
      $('.designer-dialog #designer_dialog_max_btn').hide();
    } else {
      $('.designer-dialog #designer_dialog_max_btn').html('+');
      ...
      $('.designer-dialog').attr('max', 'false');
      $('.designer-dialog #designer_dialog_max_btn').show();
    }
  ...
  showActionDialog: function (actionMetadata, action) {
    ...
    for (var k in actionMetadata.params) {
      ...
      } else if (element == 'select' || element == 'select2') {
        row = $(
          '<tr><td class="param-table-label">' +
            title +
            '</td><td><select name="' +
  ...
  _initWebsocket: function () {
    ...
    var ws_address = protocal_ws + '//' + hostname + ':' + port + '/ws/flow/websocket';
    ...
    root.socket.onmessage = function (event) {
      ...
      switch (msg.cmd) {
        case 'runtime':
          var old_is_running = 0;
          if (root.runtime) {
            if (root.runtime.is_running == 1 || root.runtime.is_running == '1') {
              old_is_running = 1;
            }
          }
          ...
          if (old_is_running==1 && root.runtime) { 
            if (root.runtime.is_running == 0 || root.runtime.is_running == '0') {
              if (root.his_option && root.his_option.event_afterstop) {
                root.his_option.event_afterstop(root.runtime);
              }
            }
          }
        ...
        case 'alert':
          ...
          if (his_option && his_option.event_message) {
            his_option.event_message(content);
          } else {
            alert(content);
          }
